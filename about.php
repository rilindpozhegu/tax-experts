<!doctype html>
<html lang="en" class="no-js">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" href="img/favicon-01.png">
    <title>Tax Experts</title>

    <link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="build/styles.css"> <!-- Resource style -->
    
</head>
<body>


<!-- Header all Pages -->    
    <?php include 'navigation_bar.php';?>


<!-- Cover Section -->

<section class="header_section_about header_title_cover">
    <div class="container">
        <div class="row">
            <h2 class=" wow fadeInUp" data-wow-duration="1s" data-wow-delay=".1s">Perspiciatis unde omnis iste natus sit <span>voluptatem accusantium</span></h2>
            <button class="button_style_c wow fadeInUp" data-wow-duration="1s" data-wow-delay=".3s">Contact</button>
        </div>
    </div>
</section>

<!-- About page Firs Section -->
<section class="about_p_section_one">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <h4 class="wow fadeInUp" data-wow-duration="1s" data-wow-delay=".2s">About Tax Expert</h4>
                <p class="wow fadeInLeft" data-wow-duration="1s" data-wow-delay=".3s">With the arrival of the value added tax in the United Arab Emirates in 2018, it is essential to be up to date and in compliance with the rules and regulations in order to keep doing business successfully. Our team of experts is ready to make sure your company adapts and transitions as smoothly as possible.</p>
                <!-- <h5 class="wow fadeInUp" data-wow-duration="1s" data-wow-delay=".2s">Tax Expert Professionals</h5> -->
                <p class="wow fadeInLeft" data-wow-duration="1s" data-wow-delay=".4s">We work to inform our clients about the VAT and its implications, help ensure that you assess the areas where VAT impacts your firm, and prepare the proper reports and procedures to ensure you comply with regulations.</p>
            </div>
        </div>
    </div>
</section>

<!-- Benefits Section -->
<section class="benefits_section benefits_section_2">
    <?php include 'benefits_section.php';?>
</section>


<!-- Homepage Our Services -->
    <?php include 'our_services_section.php';?>

<div class="container">
    <hr>
</div>


<!-- Our Expert Team -->
    <?php include 'our_expert_team_section.php';?>



<!-- Lastest News Section Everywhere -->

    <?php include 'news_section_panels.php';?>
        
<!-- Contact section input! -->

    <?php include 'contact_section_input.php';?>


<!-- Footer all Pages -->    
    <?php include 'footer_all.php';?>


<!-- General Script -->
    <script type="text/javascript" src="build/scripts.js"></script>

    
</body>
</html>